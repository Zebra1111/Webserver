...
void *read_msg(void *data)
{
	...
	alltime += (end.tv_sec - start.tv_sec) * 1000.0 + (end.tv_usec - start.tv_usec) / 1000.0;
...
void *read_file(void *data)
{
	...
	read(file_fd, address, len);
	param1->message = (char *)address;
	param1->fd = fd;
	param1->len = len;
	curtask->arg = (void *)param1;
...
void *send_msg(void *data)
{
	...
	webparam *param1 = (webparam *)data;
	write(param1->fd, param1->message, param1->len);
