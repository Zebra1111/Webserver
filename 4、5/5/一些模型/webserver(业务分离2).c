...
void addTask2readfilepool(readfilepool *pool, filename_node *curtask)
{
	push_fnamequeue(&pool->queue, curtask); // 将任务加入队列@
}
...
void *read_msg(void *data)
{
	...
	else
	{
		...
		printf("read_msg: param2->buffer:%s, buffer:%s\n", param2->buffer, buffer);
...
void *read_file(void *data)
{
	...
	param2->hit = hit;
	param2->fd = fd;
	param2->message = buffer; // 此时的buffer中是HTTP响应头部信息
	printf("read_file1: param2->message:%s, buffer:%s\n", param2->message, buffer);
	param2->ret = strlen(buffer);
	curtask->arg = (void *)param2;
	...
	while ((ret = read(file_fd, buffer, BUFSIZE)) > 0)
	{
		...
		param2->fd = fd;
		param2->message = buffer;
		printf("read_file2: param2->message:%s, buffer:%s\n", param2->message, buffer);
		param2->ret = ret;
		curtask->arg = (void *)param2;
		push_msgqueue(&sendmsg_pool->queue, curtask);
	}
...
void *send_msg(void *data)
{
	...
	printf("send_msg: param1->message:%s, message:%s\n", param1->message, message);
	long ret = param1->ret;
...
int main(int argc, char **argv)
{
	...
	readmsg_pool = initreadmsgpool(50);
	readfile_pool = initreadfilepool(50);
	sendmsg_pool = initsendmsgpool(50);
	for (hit = 1;; hit++)
	{
